            ...
            <div className={s.articleInnerWrapper}>
                <div className={s.articleDetails}>
                    <div style={{width:'40%',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                        <h10 style={{width:'95%',fontSize:'20px',color:'#1890FF'}}>{article?.title}</h10> 
...
</svg> <span style={{marginLeft:'1%'}}>{likes}</span>
                    </div>
                    <div className={s.userInfo}><span style={{display:'flex',flexDirection:'column'}}>{article?.author?.username} <span style={{fontSize:'12px',color:'grey'}}>{creationDate}</span></span> <img className={s.avatar} src={article?.author?.image ? `${article?.author?.image}` : `${image}`} alt="" srcset="" /></div>
                </div>
