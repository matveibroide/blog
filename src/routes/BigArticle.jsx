...
export default function BigArticle() {
    ...
    const description = useSelector(state => state.bigArticleReducer?.currentArticle?.description)
    const tags = useSelector(state => state.bigArticleReducer?.currentArticle?.tagList) || []
    const username = useSelector(state => state.bigArticleReducer?.currentArticle?.author?.username);
    const currentUser = useSelector(state=>state.loginReducer?.user?.user?.username)
    ...
    return (
        <>
        {!article && deleted  ? <AlertSuccess message={'Article successfully deleted'}/> : 
        <div>
            ...
            {loading ? <Spin size='large' style={{marginTop:'10%'}}/> : <div className={s.bigArticle}>
                <div className={s.articleInnerWrapper}>
                    <div className={s.articleDetails}>
                        <div style={{width:'50%',display:'flex',justifyContent:'space-around',alignItems:'left',flexDirection:'column',height:'100%'}}>
                            <div style={{display:'flex',alignItems:'center'}}>
                                <h10 style={{width:'fit-content',fontSize:'20px',color:'#1890FF',marginRight:'5px'}}>{article?.title}</h10>
                                {favorited ? 
                                <div style={{display:'flex',alignItems:'center'}} >
                                ...
                                :
                                <div  style={{display:'flex',alignItems:'center'}}>
                                    <svg onClick={() => handleClick(slug,token,favorited)} style={{marginRight:'5px'}} width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div>
                                {tags.map((item,i)=>{
                                    ...
                                    return (
                                        <span style={{marginLeft:`${margin}`,color:'red'}} key={i}>{item}</span>
                                    )
                                })}
                            </div>
                            <p style={{marginBottom:'0'}}>{description ? description : 'No description for this article'}</p>
                            </div>
                            <div className={s.userInfoWrapper}>
                                <div className={s.userInfo}>
                                    <span style={{display:'flex',flexDirection:'column'}}>{article?.author?.username}
                                    <span style={{fontSize:'12px',color:'grey'}}>{creationDate}</span></span>
                                    <img className={s.avatar} src={article?.author?.image ? `${article?.author?.image}` : `${image}`} alt=""/>
                                </div>
            
                                                {username === currentUser ? <div style={{width:'100%',display:'flex',justifyContent:'space-between'}}><button onClick={() => setIsModalOpen(true)} style={{color:'red',background:'none',padding:'3%',border:'1px solid red',borderRadius:'4px',width:'78px'}} >delete</button>
                                                <Modal title="Delete confirmation" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
                                                    <p>Are you sure to delete this article?</p>
                                                </Modal>
                                                <button style={{color:'#52C41A',background:'none',padding:'3%',border:'1px solid #52C41A',borderRadius:'4px',width:'65px'}}>edit</button></div> : null}
                            </div>
                        </div>
